...
def spectral_group(x, n, m):          # 相邻频段的组作为输入标记,以捕获和表征局部频谱特性
    ...
    for e in range(modelConfig["epoch"]):
        with tqdm(dataloader, dynamic_ncols=True) as tqdmDataLoader:
            for positive, negative in tqdmDataLoader:
                # train
                # print(dataset.target_spectrum)
                combined_vectors = np.concatenate([positive, negative, dataset.target_spectrum], axis=0) #三元组

                combined_groups = spectral_group(combined_vectors, modelConfig['band'], modelConfig['group_length']) #分频段找出局部关系
                optimizer.zero_grad() #清除梯度
                x_0 = combined_groups.to(device)  # 129,189,20
                features = net_model(x_0)   #特征提取
    ...
    for e in range(modelConfig['epoch']):
        with torch.no_grad():
            ...
            ckpt = torch.load(os.path.join(
                path, "ckpt_%s_.pt" % e), map_location=device, weights_only=True)
...
def eval(modelConfig: Dict):
    ...
    with torch.no_grad():
        ...
        model = SpectralGroupAttention(band=modelConfig['band'], m=modelConfig['group_length'],
                                       ...
                                       dim_head=modelConfig['dim_head'], mlp_dim=modelConfig['mlp_dim'], adjust=modelConfig['adjust']).to(device)
        ckpt = torch.load(os.path.join(
            path, modelConfig["test_load_weight"]), map_location=device, weights_only=True)
        model.load_state_dict(ckpt)
